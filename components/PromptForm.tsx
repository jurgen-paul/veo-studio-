...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (generationMode === GenerationMode.EXTEND_VIDEO) {
      ...
      if (model === VeoModel.VEO_LITE) {
        setModel(VeoModel.VEO_FAST);
      }
    }
    // Rule: Lite model cannot do 4k
    if (model === VeoModel.VEO_LITE && resolution === Resolution.P4K) {
      setResolution(Resolution.P1080);
    }
  }, [generationMode, model, resolution]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex flex-col">
              <CustomSelect
                ...
                {Object.values(VeoModel).map((modelValue) => (
                  <option key={modelValue} value={modelValue} disabled={modelValue === VeoModel.VEO_LITE && isExtendMode}>
                    {modelValue === VeoModel.VEO ? 'High Quality (Veo 3.1)' : modelValue === VeoModel.VEO_FAST ? 'Fast (Veo 3.1)' : 'Lite (Veo 3.1)'}
                  </option>
                ))}
              </CustomSelect>
            </div>
            ...
            <div className="flex flex-col">
              <CustomSelect
                ...
                <option value={Resolution.P4K} disabled={model === VeoModel.VEO_LITE}>4K</option>
              </CustomSelect>
              {isExtendMode ? (
                ...
                <p className="text-[10px] text-amber-400 mt-1 uppercase tracking-tighter">
                  Lite model cannot do 4k or be extended
                </p>
              ) : resolution !== Resolution.P720 && !isReferenceMode ? (
                <p className="text-[10px] text-amber-400 mt-1 uppercase tracking-tighter">
