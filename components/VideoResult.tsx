...
import {ArrowPathIcon, DownloadIcon, SparklesIcon, FileImageIcon, PlusIcon, ZapIcon, CompressIcon} from './icons';
import { compressVideo } from '../services/videoService';
// @ts-ignore
...
const VideoResult: React.FC<VideoResultProps> = ({
  ...
  const handleCompressVideo = async () => {
    if (!videoBlob) {
      alert('Video source not found for compression.');
      return;
    }
    ...
    try {
      const compressedBlob = await compressVideo(videoBlob, (progress) => {
      ...
      link.href = url;
      link.download = `veo-studio-creation-compressed.mp4`;
      ...
      alert('Video compression failed. This might be due to browser limitations or memory constraints.');
    } finally {
  ...
  return (
    <div className="w-full relative flex flex-col items-center gap-8 p-12 bg-gray-800/50 rounded-lg border border-gray-700 shadow-2xl overflow-visible">
      ...
      <div className="flex flex-wrap justify-center gap-4">
        ...
        <button
          disabled={isCompressing || isConvertingGif}
          onClick={handleCompressVideo}
          className="flex items-center gap-2 px-6 py-3 bg-purple-600 hover:bg-purple-500 text-white font-semibold rounded-lg transition-all active:scale-95 shadow-lg shadow-purple-900/20 disabled:opacity-50 disabled:cursor-wait"
          title="Reduce file size while maintaining acceptable quality"
        >
          {isCompressing ? (
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
              <span>{compressionProgress > 0 ? `${compressionProgress}%` : 'Starting...'}</span>
            </div>
          ) : (
            <>
        ...
        {canExtend ? (
          ...
          <button
            ...
            title="Lite model, 1080p, and 4k videos can't be extended">
            <SparklesIcon className="w-5 h-5" />
            Extend
          </button>
        )}
